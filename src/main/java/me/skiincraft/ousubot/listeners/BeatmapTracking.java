...
public class BeatmapTracking implements EventListener {
    ...
    public void ifExistsBeatmapSetSendMessage(long beatmapSetId, TextChannel channel) {
        try {
            ...
            OusuBot.getTrackingRepository().save(new ChannelTracking(channel, beatmapSet.getBeatmaps().get(0).getBeatmapId()));
        } catch (Exception ignored) {
        }
    }

    public void ifExistsBeatmapSendMessage(long beatmapId, TextChannel channel) {
        try {
            Endpoint endpoint = api.getAvailableTokens().getEndpoint();
            Beatmap beatmap = endpoint.getBeatmap(beatmapId).get();
            messageModel.setLanguage(Language.getGuildLanguage(channel.getGuild()));
            EmbedBuilder embed = getModelEmbedBuilder(messageModel, beatmap);
            channel.sendMessage(embed.build()).queue();
            OusuBot.getTrackingRepository().save(new ChannelTracking(channel, beatmap.getBeatmapId()));
        } catch (Exception ignored) {
        }
    }
