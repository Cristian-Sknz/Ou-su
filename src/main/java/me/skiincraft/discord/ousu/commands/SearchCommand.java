...
public class SearchCommand extends Commands {
	...
	public void action(String[] args, String label, TextChannel channel) {
		...
		if (args.length >= 1) {
			...
			MessageEmbed messageembed = TypeEmbed.LoadingEmbed().setTitle("Looking for Beatmaps").build();
			replyQueue(messageembed, message -> {
				...
				try {
					...
					for (int n : beatmaplist) {
						...
						EmbedBuilder searchembed = SearchEmbed.searchEmbed(bb.get(i), channel.getGuild());
						if (s == 0 && searchembed != null) {
							message.editMessage(new EmbedBuilder(searchembed)
									.setFooter("Loading more beatmaps").build()).queue();
							reply(new InputStreamFile(bb.get(0).getBeatmapPreview(), bb.get(0).getTitle(), ".mp3"));
							s = 1;
						}
						emb.add(searchembed);
						i++;
