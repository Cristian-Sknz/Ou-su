...
public class JSoupGetters {
    ...
    public static UserStatistics inputType(User user, LanguageManager lang) throws IOException {
        ...
        Elements timeelement = ele2.getElementsByClass("timeago");

        String firstlogin = timeelement.get(0).text().replace(" UTC", "");
        String lastActive = (timeelement.size() == 1) ? "?" : ele2.getElementsByClass("timeago").get(1).text().replace(" UTC", "");
        ...
        try {
            ...
            lastActive = (!lastActive.equals("?")) ? pretty.format(simpledate.parse(lastActive)) : "?";
