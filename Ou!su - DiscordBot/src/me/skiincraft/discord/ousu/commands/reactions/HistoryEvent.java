...
public class HistoryEvent extends ReactionsManager {
	...
	public void action(User user, TextChannel channel, String emoji) {
		...
		if (emoji.equalsIgnoreCase("◀")) {
			...
			try {
				embed = TopUserCommand.embed(new UserScores(new UserOsu(getUtils().getMsg(), GameMode.STANDARD), ScoreType.TopsScore, v));
			} catch (MalformedURLException | OsuAPIException e) {
				return;
			}
			
			ReactionMessage.osuHistory.add(new ReactionUtils(getUtils().getUser(), getUtils().getMessageID(), getUtils().getMsg(), v));
		...
		if (emoji.equalsIgnoreCase("◼")){
			...
			UserScores userss;
			try {
				userss = new UserScores(new UserOsu(getUtils().getMsg(), GameMode.STANDARD), ScoreType.TopsScore, getUtils().getValue());
				...
				channel.editMessageById(getEvent().getMessageId(), BeatMapCommand.beatmapEmbed(osuBeat).build()).queue(new Consumer<Message>() {

					@Override
					public void accept(Message message) {
						
						message.getChannel().sendFile(BeatMapCommand.idb, message.getEmbeds().get(0).getTitle() + ".mp3").queue(new Consumer<Message>() {
		...
		if (emoji.equalsIgnoreCase("▶")) {
			...
			try {
				EmbedBuilder embed = TopUserCommand.embed(new UserScores(new UserOsu(getUtils().getMsg(), GameMode.STANDARD), ScoreType.TopsScore, v));
				channel.editMessageById(getEvent().getMessageId(), embed.build()).queue();
				if (ReactionMessage.removeAudioMessage.containsKey(user)) {
					channel.deleteMessageById(ReactionMessage.removeAudioMessage.get(user)).queue();
				}
