...
public class ProfileImage extends ImageBuilder {
	...
	public void addAccuracy(Font defaultfont, int x, int y) {
		String accuracy = new DecimalFormat("#.0").format(osuUser.getAccuracy());accuracy += "%";
	...
	public void addCountry(int x, int y) {
		try {
			addImage(new URL("https://osu.ppy.sh/images/flags/" + osuUser.getCountryCode() + ".png"),
					x, y, new Dimension(96, 65));
		} catch (IOException e) {
			e.printStackTrace();
		}
