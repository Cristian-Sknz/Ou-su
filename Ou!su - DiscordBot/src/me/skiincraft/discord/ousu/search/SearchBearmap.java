...
public class SearchBearmap {
	...
	public SearchBearmap(String beatmapname) {
		...
		try {
			...
			if (receiver.getItems() == null) {
				throw new SearchNotFoundException("Não foi encontrado nenhuma pesquisa.");
			}
	...
	public List<Integer> getBeatmapSetIDs2() {
		List<Integer> i = new ArrayList<Integer>();
		for (Search s: getResults()) {
			String linkv = s.getLink().replace("https://osu.ppy.sh/beatmapsets/", "").replace("http://osu.ppy.sh/beatmapsets/", "");
			if (linkv.contains("/")) {
			int num = 0;
